...
> 작성: 2026-04-22 · 상태: Accepted (with amendments — 2026-04-29)
> 근거 문서:
...
### 2026-04-29 — Eval/CI 부분 도입 (sovereignty 표 #4 부분 변경)

**무엇**

위 sovereignty 표의 항목 #4 ("v0 스코프 확장 거절 — validator + rate limit + Upstash + Sentry + eval") 중 **eval(테스트) 와 CI 부분만** 거절을 철회. 나머지(validator·rate limit·Sentry)는 거절 유지. 동시에 Risk #5 "Eval 부재" 가 부분 완화 상태로 전환.

**왜**

원래 재검토 조건은 "위 사고 중 하나라도 발생 시". 두 번 발생:

1. **PR #13** (v0.1.1.0 → v0.1.3.0) — `SYSTEM_PROMPT` 한 줄 재서술이 `transit` 필드 누락 회귀로 드러남. 시각으로 catch 했지만 silent 회귀였다면 놓쳤을 자리.
2. **PR #53** (2026-04-29) — Flash Lite 가 `decision` 셋 중 하나만 누락한 응답을 만들 때 plan 전체가 invalid-response 로 떨어지는 자리를 코드 점검에서 발견. inline tsx 검증으로 처리했지만 영구 회귀 트랩이 필요해짐.

두 trigger 모두 메모리에 등록된 "Gemini Flash Lite 프롬프트 민감도" 패턴과 일치 — 프롬프트 한 줄 변경이 silent regression 을 만드는 환경.

**수용한 추가 의존성**

- `vitest@4.x` (devDependency 만, 빌드/런타임 영향 없음)
- GitHub Actions `build.yml` (typecheck / lint / build / test 5단계)
- 단위 테스트 41 케이스 (travel.ts 의 sanitize·decision 정규화·budget·input normalization, travel-share-store.ts 의 parseSnapshot)

**여전히 거절**

- Validator (별도 schema validation 라이브러리)
- Rate limit (per-IP, HMAC, captcha 등)
- Sentry / 외부 에러 추적
- 통합/E2E 테스트 (jsdom + Playwright 등)
- 골든셋 prompt eval (별도 sweep — TODOS 의 P2 항목으로 추적)

**관련 PR**: #59 (vitest 부트스트랩), #60 (CI 게이트)
